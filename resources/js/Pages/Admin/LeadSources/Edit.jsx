...
function AdminLeadSumbersEdit({ leadSumber }) {
    const form = useForm({ name: leadSumber.name ?? '', slug: leadSumber.slug ?? '', is_active: Boolean(leadSumber.is_active) });
    function submit(event) { event.preventDefault(); form.patch(route('admin.lead-sources.update', leadSumber.id)); }
    return <><Head title={`Edit ${leadSumber.name}`} /><div className="space-y-8"><AdminPageHeader action={<Link className="rounded-full border border-[#1E4D3A] px-4 py-2 font-body-sm text-sm font-bold text-[#1E4D3A] transition hover:bg-[#1E4D3A] hover:text-white" href={route('admin.lead-sources.show', leadSumber.id)}>Kembali</Link>} description="Perbarui nama, slug, dan status aktif lead source." eyebrow="Lead & CRM / Sumber Lead" title={`Edit ${leadSumber.name}`} /><AdminCard className="p-5"><form className="space-y-4" onSubmit={submit}><TextField error={form.errors.name} label="Nama" name="name" onChange={(event) => form.setData('name', event.target.value)} value={form.data.name} /><TextField error={form.errors.slug} label="Slug" name="slug" onChange={(event) => form.setData('slug', event.target.value)} value={form.data.slug} /><CheckboxField checked={form.data.is_active} error={form.errors.is_active} label="Aktif" onChange={(event) => form.setData('is_active', event.target.checked)} /><button className="rounded-full bg-[#1E4D3A] px-4 py-2 font-body-sm text-sm font-bold text-white transition hover:bg-[#013625] disabled:opacity-60" disabled={form.processing} type="submit">Simpan Sumber</button></form></AdminCard></div></>;
}
