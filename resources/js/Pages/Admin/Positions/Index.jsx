...
export default function Index({ positions, filters }) {
    ...
    const handleFilterChange = (newSearch, newPerPage) => {
        router.get(route('admin.positions.index'), { search: newSearch, per_page: newPerPage }, {
            preserveState: true,
            replace: true,
            preserveScroll: true
        });
    ...
    const handleEdit = (e) => {
        e.preventDefault();
        put(route('admin.positions.update', selectedPosition.id), {
            onSuccess: () => {
    ...
    return (
        <AdminLayout>
            ...
            <AdminPageHeader 
                title="Jabatan" 
                subtitle="Kelola data jabatan untuk staff lapangan"
                action={
                    <button
                ...
                {positions.data.length > 0 ? (
                            ...
                            <tbody className="divide-y divide-[#E5E7EB] font-body-sm">
                                {positions.data.map((position) => (
                                    <tr key={position.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4 font-bold text-[#333333]">{position.name}</td>
                                        <td className="px-6 py-4 text-gray-600">{position.description || '-'}</td>
                    ...
                    <div className="py-12">
                        <EmptyState 
                            ...
                            description="Belum ada data jabatan yang ditambahkan." 
                        />
                    </div>
                )}
            </AdminCard>
            ...
            <Modal show={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                <form onSubmit={handleCreate} className="p-6">
                    <h2 className="text-lg font-bold text-[#333333] mb-4">Tambah Jabatan</h2>
            ...
            <Modal show={isEditModalOpen} onClose={() => setIsEditModalOpen(false)}>
                <form onSubmit={handleEdit} className="p-6">
                    <h2 className="text-lg font-bold text-[#333333] mb-4">Edit Jabatan</h2>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Nama Jabatan</label>
                            <input
            ...
            <Modal show={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}>
                <form onSubmit={handleDelete} className="p-6">
                    <h2 className="text-lg font-bold text-[#333333] mb-4">Hapus Jabatan</h2>
                    <p className="text-sm text-gray-600">
                        Apakah Anda yakin ingin menghapus jabatan <strong>{selectedPosition?.name}</strong>?
                    </p>
                    <div className="mt-6 flex justify-end gap-3">
                        <button
                            type="button"
