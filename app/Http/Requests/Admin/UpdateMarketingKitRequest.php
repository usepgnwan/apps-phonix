...
class UpdateMarketingKitRequest extends FormRequest
{
    ...
    public function after(): array
    {
        return [
            function (Validator $validator): void {
                /** @var MarketingKit|null $kit */
                $kit = $this->route('marketingKit');
                ...
                $hasExistingFile = $kit instanceof MarketingKit && filled($kit->file_path);

                if (
                    in_array($category, [MarketingKit::CATEGORY_IMAGE, MarketingKit::CATEGORY_VIDEO, MarketingKit::CATEGORY_PDF], true)
                    && ! $hasFile
                    && ! $hasExistingFile
                ) {
